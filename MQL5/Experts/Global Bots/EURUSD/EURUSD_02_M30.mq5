...
input string CustomComment = "Strategy_6_48_116";
...
input int MomPeriod2 = 14;        //MomPeriod2
input double PriceEntryMult1 = 1.8;        //PriceEntryMult1
input int ExitAfterBars1 = 34;        //ExitAfterBars1
input double MoveSL2BE1 = 145;        //MoveSL2BE1
input double StopLossCoef1 = 2.9;        //StopLossCoef1
input int BiggestRangePeriod1 = 16;        //BiggestRangePeriod1
...
#define SMMA_1 0     //iMA(NULL,0, SMMAPeriod1, 0, MODE_SMMA, PRICE_CLOSE)
#define MOMENTUM_1 1     //iMomentum(NULL,0, MomPeriod1, PRICE_CLOSE)
#define MOMENTUM_2 2     //iMomentum(NULL,0, MomPeriod2, PRICE_CLOSE)
#define ATR_1 3     //iCustom(NULL, 0, "SqATR", 20)
...
string sqStrategyName = "Strategy 6.48.116";
...
if (_sqIsBarOpen == true) {
     ...
     LongEntrySignal = (NormalizeDouble((double) sqGetAsk("Current"), 6) >= NormalizeDouble((double) sqGetIndicatorValue(SMMA_1, 0, 1), 6));

     ShortEntrySignal = (NormalizeDouble((double) sqGetBid("Current"), 6) <= NormalizeDouble((double) sqGetIndicatorValue(SMMA_1, 0, 1), 6));

     LongExitSignal = (((sqIsFalling("sqHigh(NULL,0, 1)", 2, true, 0 + 1, 0))
      &&   sqIsRising(MOMENTUM_1, 2, false, 1))
      &&   sqIsRising(MOMENTUM_2, 2, false, 3));

     ShortExitSignal = (((sqIsRising("sqLow(NULL,0, 1)", 2, true, 0 + 1, 0))
      &&   sqIsFalling(MOMENTUM_1, 2, false, 1))
      &&   sqIsFalling(MOMENTUM_2, 2, false, 3));
...
//------------------------
...
)
   {
      // Action #1
        // Enter at Stop
      openPrice = sqFixMarketPrice((sqGetAsk("Current") + (PriceEntryMult1 * sqBiggestRange("NULL",0, BiggestRangePeriod1, 1))), "Current");
...
//------------------------
...
)
   {
      // Action #1
        // Enter at Stop
      openPrice = sqFixMarketPrice((sqGetBid("Current") - (PriceEntryMult1 * sqBiggestRange("NULL",0, BiggestRangePeriod1, 1))), "Current");
...
bool initIndicators(){
   ...
   ArrayResize(indicatorHandles, ArraySize(indicatorHandles) + 1, 10);
   indicatorHandles[MOMENTUM_2] = iMomentum(NULL,0, MomPeriod2, PRICE_CLOSE);
...
double sqGetExpressionByIdentification(string id, int shift) {
      if(id == "sqHigh(NULL,0, 1)") {
          return   sqHigh(NULL,0, shift + 1);
      }
      if(id == "sqLow(NULL,0, 1)") {
          return   sqLow(NULL,0, shift + 1);
      }
