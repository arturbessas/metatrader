...
void sendIncreaseAndUpdateTp()
{
    ...
    {
        if (opType == "buy")
            trade.BuyLimit(increasesQtt[increaseStep], roundPrice(entryPrice - increasesPts[increaseStep]), stock, 0, 0, ORDER_TIME_GTC, 0, "Aumento de posicao " + string(increaseStep));
        else if (opType == "sell")
            trade.SellLimit(increasesQtt[increaseStep], roundPrice(entryPrice + increasesPts[increaseStep]), stock, 0, 0, ORDER_TIME_GTC, 0, "Aumento de posicao " + string(increaseStep));
...
void sendTP()
{
    ...
    // gain no zero a zero em caso de aumento
    //if(posManager.Volume() > 1)
    //	tp = posManager.PriceOpen();

    if (opType == "buy")
        trade.SellLimit(posManager.Volume(), roundPrice(tp), stock, 0, 0, ORDER_TIME_GTC, 0, "Take Profit");
    else if (opType == "sell")
        trade.BuyLimit(posManager.Volume(), roundPrice(tp), stock, 0, 0, ORDER_TIME_GTC, 0, "Take Profit");
