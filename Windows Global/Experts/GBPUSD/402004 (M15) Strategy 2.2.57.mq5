...
input string CustomComment = "_M15_Strategy_2_2_57";
...
#define LOWESTINRANGE_1 0     //iCustom(NULL,0, "SqLowestInRange", "23:30", "5:30")
#define EMA_1 1     //iMA(NULL,0, 22, 0, MODE_EMA, PRICE_CLOSE)
#define HIGHESTINRANGE_1 2     //iCustom(NULL,0, "SqHighestInRange", "23:30", "5:30")
...
string sqStrategyName = "(M15) Strategy 2.2.57";
...
if (_sqIsBarOpen == true) {
     ...
     LongEntrySignal = ((sqIsRising(LOWESTINRANGE_1, 15, true, 2 + 3, 0))
      &&   (NormalizeDouble((double) sqGetAsk("Current"), 6) < NormalizeDouble((double) sqGetIndicatorValue(EMA_1, 0, 1), 6)));

     ShortEntrySignal = ((sqIsFalling(HIGHESTINRANGE_1, 15, true, 2 + 3, 0))
      &&   (NormalizeDouble((double) sqGetBid("Current"), 6) > NormalizeDouble((double) sqGetIndicatorValue(EMA_1, 0, 1), 6)));
...
//------------------------
...
)
   {
      ...
      sl = sqFixMarketPrice(sqGetSLPercentLevel("Current", ORDER_TYPE_BUY, openPrice, 1, 0.46), "Current");
      pt = sqFixMarketPrice(sqGetPTPercentLevel("Current", ORDER_TYPE_BUY, openPrice, 1, 1.39), "Current");
      ...
      if(_ticket > 0) {
         ...
         sqSetTrailingStop(_ticket, "224", SLPTTYPE_RANGE);
         sqSetTSActivation(_ticket, "30");
...
//------------------------
...
)
   {
      // Action #1
        // Enter at Market
      openPrice = 0;  //Open at current market price
      sl = sqFixMarketPrice(sqGetSLPercentLevel("Current", ORDER_TYPE_SELL, openPrice, 1, 0.46), "Current");
      pt = sqFixMarketPrice(sqGetPTPercentLevel("Current", ORDER_TYPE_SELL, openPrice, 1, 1.39), "Current");
      size = mmLots * mmMultiplier;
...
bool initIndicators(){
   ...
   indicatorHandles[LOWESTINRANGE_1] = iCustom(NULL,0, "SqLowestInRange", "23:30", "5:30");
   
   ArrayResize(indicatorHandles, ArraySize(indicatorHandles) + 1, 10);
   indicatorHandles[EMA_1] = iMA(NULL,0, 22, 0, MODE_EMA, PRICE_CLOSE);
   
   ArrayResize(indicatorHandles, ArraySize(indicatorHandles) + 1, 10);
   indicatorHandles[HIGHESTINRANGE_1] = iCustom(NULL,0, "SqHighestInRange", "23:30", "5:30");
...
double sqGetValueByIdentification(int idHash) {
   ...
   if(idHash == sqStringHash("224")) {
      return (sqConvertToRealPips(PositionGetString(POSITION_SYMBOL), 224));
   }
   if(idHash == sqStringHash("30")) {
      return (sqConvertToRealPips(PositionGetString(POSITION_SYMBOL), 30));
   }
   if(idHash == sqStringHash("224")) {
      return (sqConvertToRealPips(PositionGetString(POSITION_SYMBOL), 224));
   }
