//+------------------------------------------------------------------+
//| (M30) Strategy 4.2.82 EA
//|
//|    Generated by StrategyQuant X Build 141 for MetaTrader5 (hedged)
//|    Generated at 02/19/2025 19:39
//| 
//|    Backtested on GBPUSD_ZERO / M30, 2021.06.02 - 2025.02.18 
//+------------------------------------------------------------------+
...
input string CustomComment = "_M30_Strategy_4_2_82";
...
#define MTKELTNERCHANNEL_1 0     //iCustom(NULL,0, "SqMTKeltnerChannel", 47, 2.0)
#define LOWESTINRANGE_1 1     //iCustom(NULL,0, "SqLowestInRange", "17:00", "7:00")
#define QQE_1 2     //iCustom(NULL,0, "SqQQE", 7, 4, 86.350, 0)
#define MTKELTNERCHANNEL_2 3     //iCustom(NULL,0, "SqMTKeltnerChannel", 47, 2)
#define HIGHESTINRANGE_1 4     //iCustom(NULL,0, "SqHighestInRange", "17:00", "7:00")
#define QQE_2 5     //iCustom(NULL,0, "SqQQE", 7, 4, 86.35, 0)
...
string sqStrategyName = "(M30) Strategy 4.2.82";
...
if (_sqIsBarOpen == true) {
     ...
     LongEntrySignal = ((NormalizeDouble((double) sqGetIndicatorValue(MTKELTNERCHANNEL_1, 0, 1), 6) < NormalizeDouble((double) sqGetIndicatorValue(LOWESTINRANGE_1, 0, 2), 6))
      &&   (NormalizeDouble((double) sqGetIndicatorValue(QQE_1, 0, 3), 6) < NormalizeDouble((double) 70.0, 6)));

     ShortEntrySignal = ((NormalizeDouble((double) sqGetIndicatorValue(MTKELTNERCHANNEL_2, 1, 1), 6) > NormalizeDouble((double) sqGetIndicatorValue(HIGHESTINRANGE_1, 0, 2), 6))
      &&   (NormalizeDouble((double) sqGetIndicatorValue(QQE_2, 0, 3), 6) > NormalizeDouble((double) 30, 6)));
...
//------------------------
...
)
   {
      ...
      sl = sqFixMarketPrice(sqGetSLPercentLevel("Current", ORDER_TYPE_BUY, openPrice, 1, 0.73), "Current");
      pt = sqFixMarketPrice(sqGetPTPercentLevel("Current", ORDER_TYPE_BUY, openPrice, 1, 1.1), "Current");
      ...
      if(_ticket > 0) {
         ...
         sqSetMoveSL2BE(_ticket, "48", SLPTTYPE_RANGE);

         // TrailingStop initialization
         sqSetTrailingStop(_ticket, "150", SLPTTYPE_RANGE);
...
//------------------------
...
)
   {
      // Action #1
        // Enter at Market
      openPrice = 0;  //Open at current market price
      sl = sqFixMarketPrice(sqGetSLPercentLevel("Current", ORDER_TYPE_SELL, openPrice, 1, 0.73), "Current");
      pt = sqFixMarketPrice(sqGetPTPercentLevel("Current", ORDER_TYPE_SELL, openPrice, 1, 1.1), "Current");
      size = mmLots * mmMultiplier;
...
bool initIndicators(){
   ...
   indicatorHandles[MTKELTNERCHANNEL_1] = iCustom(NULL,0, "SqMTKeltnerChannel", 47, 2.0);
   
   ArrayResize(indicatorHandles, ArraySize(indicatorHandles) + 1, 10);
   indicatorHandles[LOWESTINRANGE_1] = iCustom(NULL,0, "SqLowestInRange", "17:00", "7:00");
   
   ArrayResize(indicatorHandles, ArraySize(indicatorHandles) + 1, 10);
   indicatorHandles[QQE_1] = iCustom(NULL,0, "SqQQE", 7, 4, 86.350, 0);
   
   ArrayResize(indicatorHandles, ArraySize(indicatorHandles) + 1, 10);
   indicatorHandles[MTKELTNERCHANNEL_2] = iCustom(NULL,0, "SqMTKeltnerChannel", 47, 2);
   
   ArrayResize(indicatorHandles, ArraySize(indicatorHandles) + 1, 10);
   indicatorHandles[HIGHESTINRANGE_1] = iCustom(NULL,0, "SqHighestInRange", "17:00", "7:00");
   
   ArrayResize(indicatorHandles, ArraySize(indicatorHandles) + 1, 10);
   indicatorHandles[QQE_2] = iCustom(NULL,0, "SqQQE", 7, 4, 86.35, 0);
...
double sqGetValueByIdentification(int idHash) {
   ...
   if(idHash == sqStringHash("48")) {
      return (sqConvertToRealPips(PositionGetString(POSITION_SYMBOL), 48));
   }
   if(idHash == sqStringHash("150")) {
      return (sqConvertToRealPips(PositionGetString(POSITION_SYMBOL), 150));
   }
   if(idHash == sqStringHash("48")) {
      return (sqConvertToRealPips(PositionGetString(POSITION_SYMBOL), 48));
   }
